...
public class Ball extends Entity {
	...
	public void update(float deltaTime, ArrayList<PhysicPlayer> players, boolean launch, Room room) {
		...
		if (heldPlayer >= 0) {
			PhysicPlayer player = players.get(heldPlayer);
			...
			if (launch) {
				movement.set((float) (-Math.sin(-player.playerRotation.getYaw() / 180 * Math.PI)), (float) (Math.sin(player.playerRotation.getPitch() / 180 * Math.PI)),
						(float) (-Math.cos(-player.playerRotation.getYaw() / 180 * Math.PI)));
				movement.scl(0.5f);
